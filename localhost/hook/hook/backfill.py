...
(`install.sh --backup` excludes the vectors, which are incompressible and
dominate the dump), or after switching embedding provider. Re-embeds every
message whose embedding IS NULL using the same provider/model as the live hook
(hook.embed — Ollama / mxbai-embed-large by default), then rebuilds the ivfflat
index so its centroids are trained on the full, populated vector set rather than
an empty table.
